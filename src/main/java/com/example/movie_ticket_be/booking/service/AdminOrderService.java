...
public class AdminOrderService {
	...
	public OrderResponse getOrderDetail(Long orderId) {
		...
		return OrderResponse.builder().orderId(order.getOrderId()).userId(userId).fullName(fullName)
				...
				.discountAmount(order.getDiscountAmount()).promotionCode(order.getPromotionCode())
				.finalPrice(order.getFinalPrice()).pointsEarned(order.getPointsEarned())
	...
	public AdminOrderStatsResponse getStats(LocalDateTime from, LocalDateTime to) {
		BigDecimal totalRevenue = Optional.ofNullable(orderRepository.sumRevenue(OrderStatus.PAID, from, to))
				.orElse(BigDecimal.ZERO);
		return AdminOrderStatsResponse.builder().totalRevenue(totalRevenue)
				...
				.usedOrders(orderRepository.countByStatusAndPeriod(OrderStatus.USED, from, to)).build();
	}
	...
	private ShowTimeInfo buildShowTimeInfo(Orders order) {
		...
		return ShowTimeInfo.builder().movieName(st.getMovies() != null ? st.getMovies().getTitle() : null)
				.roomName(roomName).showTime(st.getStartTime()).cinemaName(cinemaName).cinemaAddress(cinemaAddress)
				.build();
	}
	...
	private List<OrderFoodResponse> buildFoodResponses(Orders order) {
		...
		return order.getOrderFoods().stream()
				.map(f -> OrderFoodResponse.builder().foodId(f.getFoods().getFoodId()).name(f.getFoods().getName())
						.quantity(f.getQuantity()).unitPrice(f.getUnitPrice()).totalPrice(f.getTotalPrice()).build())
				.toList();
	}
	...
	private AdminOrderSummaryResponse toSummaryResponse(Orders order) {
		...
		return AdminOrderSummaryResponse.builder().orderId(order.getOrderId()).userId(userId).fullName(fullName)
				.movieName(movieName).cinemaName(cinemaName).showTime(showTime).ticketCount(ticketCount)
				.finalPrice(order.getFinalPrice()).orderStatus(order.getOrderStatus())
				.bookingTime(order.getBookingTime()).build();
	}
