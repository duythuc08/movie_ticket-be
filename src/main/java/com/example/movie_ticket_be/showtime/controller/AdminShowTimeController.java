...
public class AdminShowTimeController {
	...
	public ApiResponse<ShowTimeResponse> updateShowTime(@PathVariable Long id,
		...
		return ApiResponse.<ShowTimeResponse>builder().result(adminShowTimeService.updateShowTime(id, request))
				.message("Cập nhật suất chiếu thành công").build();
	}

	@PutMapping("/{id}/cancel")
	@PreAuthorize("hasRole('ADMIN')")
	public ApiResponse<ShowTimeResponse> cancelShowTime(@PathVariable Long id) {
		return ApiResponse.<ShowTimeResponse>builder().result(adminShowTimeService.cancelShowTime(id))
				.message("Hủy suất chiếu thành công").build();
	}
	...
	public ApiResponse<Page<ShowTimeResponse>> getAdminShowTimes(
		...
		return ApiResponse.<Page<ShowTimeResponse>>builder()
				...
				.message("Lấy danh sách suất chiếu thành công").build();
	}

	@GetMapping("/{id}")
	@PreAuthorize("hasRole('ADMIN')")
	public ApiResponse<ShowTimeDetailResponse> getAdminShowTimeDetail(@PathVariable Long id) {
		return ApiResponse.<ShowTimeDetailResponse>builder().result(adminShowTimeService.getAdminShowTimeDetail(id))
				.message("Lấy chi tiết suất chiếu thành công").build();
	}

	@GetMapping("/gantt")
	@PreAuthorize("hasRole('ADMIN')")
	public ApiResponse<List<ShowTimeResponse>> getShowTimeForGanttChart(@RequestParam Long cinemaId,
			@RequestParam LocalDate day) {
		return ApiResponse.<List<ShowTimeResponse>>builder()
				.result(adminShowTimeService.getShowTimeForGanttChart(cinemaId, day))
				.message("Lấy lịch chiếu Gantt thành công").build();
	}
}
