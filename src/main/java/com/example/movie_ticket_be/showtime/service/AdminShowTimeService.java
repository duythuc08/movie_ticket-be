...
public class AdminShowTimeService {
    ...
    public List<ShowTimeResponse> createShowTimes(ShowTimeRequest request) {
        ...
        List<LocalDateTime> sortedStarts = request.getStartTimes().stream()
                .sorted()
                .distinct()
                .toList();
        ...
        List<ShowTimes> toSave = sortedStarts.stream()
                .map(start -> {
                    ShowTimes st = ShowTimes.builder()
                            .startTime(start)
                            .endTime(roundUpToNearest5(start.plusMinutes(totalMinutes)))
                            .rooms(room)
                            .movies(movie)
        ...
        saved.forEach(st -> {
            if (request.getPrices() != null && !request.getPrices().isEmpty()) {
                List<ShowTimePriceRequest> priceRequests = request.getPrices().stream()
                        .map(p -> ShowTimePriceRequest.builder()
    ...
    public ShowTimeResponse updateShowTime(Long showTimeId, UpdateShowTimeRequest request) {
        ...
        checkRoomTimeConflict(showTime.getRooms().getRoomId(), showTime.getStartTime(), showTime.getEndTime(), showTimeId);
    ...
    private static LocalDateTime roundUpToNearest5(LocalDateTime dt) {
        ...
        if (remainder == 0) return dt.withSecond(0).withNano(0);
    ...
    private void checkRoomTimeConflict(Long roomId, LocalDateTime start, LocalDateTime end, Long excludeId) {
        ...
        boolean gapViolation = excludeId == null
                ? showTimeRepository.existsConflictingShowtime(roomId, start.minusMinutes(CLEANING_MINUTES), end.plusMinutes(CLEANING_MINUTES))
                : showTimeRepository.existsConflictingShowtimeExcluding(roomId, start.minusMinutes(CLEANING_MINUTES), end.plusMinutes(CLEANING_MINUTES), excludeId);
        if (gapViolation) {
            throw new AppException(ErrorCode.SHOWTIME_BUFFER_CONFLICT);
        }
    }
