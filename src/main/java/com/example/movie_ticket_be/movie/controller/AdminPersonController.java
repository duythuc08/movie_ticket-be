...
public class AdminPersonController {
	...
	public ApiResponse<PersonResponse> createPerson(@RequestBody PersonRequest request) {
		return ApiResponse.<PersonResponse>builder().result(adminPersonService.createPerson(request))
				.message("Thêm người thành công").build();
	}

	@PostMapping("/bulk")
	@PreAuthorize("hasRole('ADMIN')")
	public ApiResponse<List<PersonResponse>> createPersons(@RequestBody List<PersonRequest> requests) {
		return ApiResponse.<List<PersonResponse>>builder().result(adminPersonService.createPersons(requests))
				.message("Đã thêm thành công " + requests.size()).build();
	}
	...
	public ApiResponse<Page<PersonResponse>> getAllPersons(
		...
		return ApiResponse.<Page<PersonResponse>>builder().result(adminPersonService.getAllPersons(spec, pageable))
				.message("Lấy danh sách người thành công").build();
	}

	@GetMapping("/{id}")
	@PreAuthorize("hasRole('ADMIN')")
	public ApiResponse<PersonResponse> getPersonById(@PathVariable long id) {
		return ApiResponse.<PersonResponse>builder().result(adminPersonService.getPersonById(id))
