...
public class AdminMovieController {
	...
	ApiResponse<MovieResponse> createAdminMovie(@RequestBody @Valid MovieCreationRequest request) {
		return ApiResponse.<MovieResponse>builder().result(adminMovieService.createAdminMovie(request))
				.message("Thêm phim mới thành công").build();
	}
	...
	public ApiResponse<Page<AdminMovieResponse>> getAdminMovies(
		...
		return ApiResponse.<Page<AdminMovieResponse>>builder().result(adminMovieService.getAdminMovies(spec, pageable))
				.message("Lấy danh sách phim thành công").build();
	}

	@GetMapping("/{id}")
	@PreAuthorize("hasRole('ADMIN')")
	public ApiResponse<AdminMovieResponse> getAdminMovieDetail(@PathVariable long id) {
		return ApiResponse.<AdminMovieResponse>builder().result(adminMovieService.getAdminMovieDetail(id))
				.message("Lấy thông tin phim thành công").build();
	}

	@PutMapping("/{id}")
	@PreAuthorize("hasRole('ADMIN')")
	public ApiResponse<AdminMovieResponse> updateAdminMovie(@PathVariable long id,
			@RequestBody @Valid MovieUpdateRequest request) {
		return ApiResponse.<AdminMovieResponse>builder().result(adminMovieService.updateAdminMovie(id, request))
				.message("Cập nhật phim thành công").build();
	}
