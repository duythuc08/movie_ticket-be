...
public class UserPromotionService {
    ...
    public List<PublicPromotionResponse> getAvailablePromotions() {
        ...
        return promotionRepository.findByStatus(PromotionStatus.PUBLISHED).stream()
                ...
                .filter(p -> !userPromotionRepository.existsByUsers_UserIdAndPromotion_PromotionId(userId, p.getPromotionId()))
    ...
    public void claimPromotion(Long promotionId) {
        ...
        userPromotionRepository.save(UserPromotion.builder()
                .users(user)
                .promotion(promotion)
                .isUsed(false)
                .build());
    ...
    public List<UserVoucherResponse> getMyVouchers() {
        ...
        return userPromotionRepository.findByUsers_UserIdAndIsUsedFalse(userId).stream()
                .map(up -> toVoucherResponse(up, null))
                .collect(Collectors.toList());
    }
    ...
    private boolean checkEligible(Promotion p, Long movieId, BigDecimal totalAmount, LocalDateTime now, DayOfWeek today) {
        if (p.getStatus() != PromotionStatus.PUBLISHED) return false;
        if (now.isAfter(p.getEndTime()) || now.isBefore(p.getStartTime())) return false;
        if (p.getUseLimit() != null && p.getUsedCount() >= p.getUseLimit()) return false;
        if (p.getMinOrderValue() != null && totalAmount != null
                && totalAmount.compareTo(p.getMinOrderValue()) < 0) return false;
        if (p.getApplicableMovies() != null && !p.getApplicableMovies().isEmpty() && movieId != null) {
            boolean match = p.getApplicableMovies().stream()
                    .anyMatch(m -> m.getMovieId().equals(movieId));
            if (!match) return false;
        }
        if (p.getDayOfWeek() != null && !p.getDayOfWeek().isEmpty()) {
            if (!p.getDayOfWeek().contains(today)) return false;
        }
        return true;
    }
    ...
    private PublicPromotionResponse toPublicResponse(Promotion p) {
        return PublicPromotionResponse.builder()
                ...
                .useLimit(p.getUseLimit())
                .usedCount(p.getUsedCount())
                .startTime(p.getStartTime())
                .endTime(p.getEndTime())
                .dayOfWeek(p.getDayOfWeek())
                .applicableMovieIds(toMovieIds(p.getApplicableMovies()))
                .build();
    }
