...
public class AuthenticationService {
	...
	public void forgotPassword(String email) {
		Users user = userRepository.findByUsername(email).orElseThrow(() -> new AppException(ErrorCode.USER_NOT_FOUND));

		String otp = generateVerificationCode();
		VerificationToken token = verificationTokenRepository.findByUser(user).orElse(new VerificationToken());
	...
	private String generateToken(Users user) {
		...
		JWTClaimsSet jwtClaimsSet = new JWTClaimsSet.Builder().subject(user.getUsername()).issuer("infinity_cinema.com")
				.issueTime(new Date())
				.expirationTime(new Date(Instant.now().plus(30, ChronoUnit.MINUTES).toEpochMilli()))
				.claim("userId", user.getUserId()).claim("scope", buildScope(user)).jwtID(UUID.randomUUID().toString())
				.build();
	...
	public void verifyEmail(String otp, String userName) {
		...
		VerificationToken verificationToken = verificationTokenRepository.findByUserAndVerificationCode(user, otp)
	...
	public void resendOtp(String userName) {
		...
		if (token.getLastSentAt() != null && token.getLastSentAt().isAfter(LocalDateTime.now().minusSeconds(30))) {
